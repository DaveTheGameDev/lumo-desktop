...
- **Audio / microphone**: the sandbox includes `--socket=pulseaudio` so Lumo's voice input can work. If you never use voice and want a tighter sandbox, revoke it for the installed app without rebuilding: `flatpak override --user --nosocket=pulseaudio io.github.davethegamedev.LumoDesktop` (undo with `--socket=pulseaudio` in place of `--nosocket=pulseaudio`).
